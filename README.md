...
│   └── tests/
│       ├── cases.tsv                     172 case (run-all과 1:1 정합, 100% 구현)
...
│   ├── verify-setup.sh                   §6.3 file/structure 체크 (현재 77 PASS)
...
- Hook 단위 테스트: `hooks/tests/cases.tsv` (172 케이스, run-all과 1:1 정합, 100% 통과). 원 설계 명세(spec §6.2, 원안 65개)
