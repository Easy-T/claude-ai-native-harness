...
- **검증자(review-strict)의 기준선은 임무-분리다**(C16, spec §15.1) — **준수-확인**(Workflow 경로·Rule C2) = **작업자 티어**(상속·동적 실행자는 세션 티어로 평가, 실행자 전무 시 세션 폴백) / **판단-게이트**(Agent 경로·Rule B) = **`max(세션 티어, 작업자 티어)` 유지**. 하한: 어떤 임무에서도 검증자 < 작업자 금지. 판단-게이트 경로에서 `model: inherit` 유지가 그중 세션 축을 공짜로 보장한다(frontmatter 실물 명시 `inherit`; C13 정밀화(C16 §15.1이 임무-분리로 재정의 — Workflow 준수-확인 경로는 작업자 티어) — 종전 "≥ 작업자 티어" 단독 표현은 실행자를 세션 위로 상향한 경우만 커버해 세션 축이 빠졌고, model-policy.md의 "≥ 세션" 표현은 그 반대였다. 두 독법의 합집합이 기준선이다 — spec §12.1). 실행자를 세션 티어 위로 상향했다면 **검증자도 그 티어 이상**이어야 한다. 실행자·탐색자 기본값은 `docs/ai-context/model-policy.md` 매트릭스가 별도 규정. **기준선 미만 하향은 사유 선언 필수**(DOWNGRADE-DECLARED 동형). 오케스트레이터의 동적 모델 선택은 채택하지 않는다 — 가장 신뢰하지 않는 계층(모델 판단)에 검증자 선택 재량을 주면 self-pass 우회로가 열린다.
...
**내부 적대 패스 (C16 §15.2)**: senior review(closeout-pr-cycle Phase 4)는 refute-by-default 적대 임무로 운용한다 — C16 블라인드 A/B(재발견 7/13, n=1 방향 신호)가 임무 프레이밍 전환의 충분성을 보임. 티어=세션 상속(판단-게이트 floor)·PASS/FAIL 출력 계약 불변(프레이밍만 교체). GPT-전속 발견 6건의 실측 1례처럼 교차패밀리 층의 대체가 아니라 보완이다.
