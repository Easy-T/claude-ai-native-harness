...
5. **모델ID `[1m]` suffix**: settings.json의 모델명에 붙은 `[1m]`은 1M 컨텍스트 창 인식용 워크어라운드(wire 전송 전 strip됨). Claude Code 업데이트가 이를 정규화·제거할 수 있으니(v2.1.173 선례) 업그레이드 후 `/context`로 창이 1M인지 재검증. 200K로 붕괴하면 auto-compact가 조기 발화해 무인 루프가 죽는다.
   - **rot-정렬 트리거 정책(C6/GAP-018)**: `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`는 **40**(1M 기준 400K)이 기본 — rot 실증(~300-400K)·dumb-zone(40%, 02 §4)에 정렬해 트리거가 rot **이전**에 발동(D3 L4 conjunct). auto-compact-watch 경고는 `WARN=PCT−10`=30%=300K로 파생돼 rot 진입 前 heads-up. **settings.example=40은 tracked(seal #39가 ≤40 봉인)** — 되돌리면 verify-setup FAIL.
