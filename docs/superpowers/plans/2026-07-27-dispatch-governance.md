...
**Best-Direction Check:** 최선안 = Workflow 스크립트를 **실제 JS 파서로 AST 수준 분석**해 스폰 객체별 model/agentType을 정확히 추출 / 채택안 = **node 파서로 스폰 객체를 개별 분리**해 per-spawn 평가.
→ `DOWNGRADE-DECLARED(정직 공개 유지)`: AST 파싱은 ①hook이 임의 사용자 스크립트를 파싱하게 되어 파서 자체가 공격면이 되고 ②동적 조립(`'execute'+'-strict'`)은 AST로도 미해결이라 **잔여가 동일하게 남으며** ③advisory hook의 복잡도·지연 상한을 넘는다. 채택안은 "스크립트당 1회 판정 → 스폰당 1회 판정"으로 **탐지 입도를 한 단계 올리는 것**이 목표이고, 정적-텍스트 분석이라는 성질은 §10·§12.3에 이미 정직 공개된 채로 **유지**된다(은폐 아님). 난이도 회피가 아니라 위협 모델(적대 우회가 아닌 망각) 대비 적정선 판단.

**★채택안 실물 정정 (Task 7, Closeout)**: 최초 구현은 **정규식 휴리스틱**이었고, GPT 교차패밀리 리뷰가 그 방식의 결함 10건을 적발했다(spec §12.6 [A]) — 특히 주석·중첩 템플릿의 `model` 리터럴이 실제 무선언 스폰을 가려 **Rule C 를 침묵**시켰다. Task 7 에서 채택안을 **렉서(주석/문자열/템플릿/정규식 마스킹) + opts 프로퍼티 워크**로 교체했다.
**이 교체는 열화가 아니라 강화**이며, 위 AST 미채택 근거 ①~③은 **그대로 유효**하다: 렉서는 어휘 분석까지만 하고 구문 트리를 만들지 않으므로 평가·해석 경로가 없어 공격면이 늘지 않고, 동적 조립 잔여도 동일하게 남는다(값을 `-`/`*` 로 보고 = 안전 방향). 따라서 DOWNGRADE-DECLARED 의 범위와 사유는 변경 없이 유지되며, 다만 위 문장의 "정규식 파서" 표현이 실물과 어긋나므로 "node 파서"로 정정한다.
