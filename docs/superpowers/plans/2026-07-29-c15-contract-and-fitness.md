...
- [ ] **Step 1: RED — 신규 픽스처 2개 + 186 기대값 변경을 먼저 적용**
...
- [ ] **Step 3: GREEN — 방출부 1곳 + 헤더 계약 서술 갱신**
...
- [ ] **Step 4: GREEN 확인 (hook E2E는 Task 2에서 — 이 시점 smp 픽스처는 기존 그대로 통과해야 함: C3는 `-`/`inherit`만 매치라 `*` 무영향)**

Run: `bash hooks/tests/run-all.sh 2>&1 | tail -4`
Expected: `246 / 246 passed` (244+2), cases.tsv 정합 OK. 통과 출력 원문 인용.

- [ ] **Step 5: README 카운트 갱신 + 커밋**
...
- [ ] **Step 5: cases.tsv 3건 + README 카운트(249) + 커밋**
...
- [ ] **Step 2: 검증 — 문서 self-consistency grep**
...
- [ ] **Step 3: GREEN — active-plan 블록에 stdout 1줄**
...
- [ ] **Step 4: GREEN 확인 + 무회귀 (기존 ssa 01-07 케이스 그대로)**

Run: `bash hooks/tests/run-all.sh 2>&1 | tail -4`
Expected: `251 / 251 passed`.

- [ ] **Step 5: cases.tsv 2건 + README 카운트(251) + 커밋**
...
- [ ] **Step 1: Skill 도구로 `improve-codebase-architecture` 호출** (메인 세션 — phase-skills invoked 선언 대상)
- [ ] **Step 2: Phase 1 탐색은 explore-strict 위임(skill 절차), Phase 2 후보 목록(스캐폴드 프루닝 + 메모리 수명주기 포함) 산출**
- [ ] **Step 3: 후보 보고를 Closeout 보고에 포함. Phase 3(실행)은 사용자 승인 후 별도 사이클 — 이번엔 미실행. Phase 4(README)는 하네스 README가 이미 존재·현행이므로 "갱신 불필요" 판정 기록(신규 생성 아님)**
