...
- [ ] doctor REQUIRED_HOOKS+enforce-session-budget.sh · verify-setup #8 loop+주석 10→11 · install.sh REQUIRED · README `10개 hook`→11+표행
...
- [ ] run-all: sb-180(unset→0)·sb-181(under→0)·sb-182(over→2, 카운터 프리시드)·sb-183(GOAL_BUDGET_SKIP+over→0) — BUDGET_DIR override hermetic, 구현 전 RED(hook 부재→127/미차단)
- [ ] cases.tsv 4행 + README 159→163
