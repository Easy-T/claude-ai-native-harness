...
- [x] `[ -z "${SESSION_TOOL_BUDGET:-}" ] && exit 0`(source 전, 최소 off-비용) → source _common.sh → 숫자검증(비숫자/≤0→exit 0) → require_node → json_get_many session_id tool_name → SID 부재 exit 0
- [x] 카운터 `${BUDGET_DIR:-$HOME/.claude/hooks/.budget}/<sid>` 증분(비숫자→0 리셋) · export RL_SID/RL_TOOL(run-log)
- [x] GOAL_BUDGET_SKIP→hook_log PASS skip:+surface_bypass+exit 0 / CNT>budget→hook_log BLOCK+stderr(GOAL_BUDGET_SKIP·상향 안내)+exit 2 / CNT==ceil(budget*0.8) 최초→hook_log ALERT budget-80pct+emit_additional_context(session_marker 1회)+exit 0 / else exit 0(silent 증분)
...
- [x] **settings.example.json**(tracked): PreToolUse `*` budget group 배선 완료. **라이브 settings.json은 머지 후**(hook 파일 도착 시) 배선 — 현 세션서 배선하면 없는 파일 참조로 매 호출 오류. staged 검증은 budget-포함 합성 settings.json으로 #23 parity 증명.
- [x] doctor REQUIRED_HOOKS+enforce-session-budget.sh · verify-setup #8 loop+주석 10→11 · install.sh REQUIRED · README `10개 hook`→11+표행 · verify-setup #14 주석 정정(새 `*` matcher는 W|E|N/Bash 분해 밖 — cosmetic, FLAG NOTE)
...
- [x] session-start-audit: stale `.budget/*`·`.budget/.warned-*` prune(7일+, 기존 marker prune 인접)
- [x] run-all: sb-180(unset→exit 0)·sb-181(under→exit 0)·sb-182(over→exit 2, 카운터 프리시드)·sb-183(GOAL_BUDGET_SKIP+over→exit 0)·**sb-184(80% 경고 방출 단언 — additionalContext 출력 검사, FLAG A: 경고 브랜치 미테스트 방지)** — BUDGET_DIR override hermetic, 구현 전 RED
- [x] cases.tsv 5행 + README 159→164

### Task 4: 검증 + Closeout
- [x] staged run-all **164/164** · verify-setup **73/0**(#8=11 hooks·#23 parity·#24 doctor⊇hooks·#36 README 73 동기; 72→73은 #8 loop +1) · 5 시나리오 직접 실측 GREEN(sb-180~184) · verify-all은 머지 후 라이브 배선과 함께 최종 확증
- [x] 03 D5 재채점(2→3: 결정론 iteration ceiling 착륙; L4=슬라이싱 잔여·L5 sandbox=GAP-007) · 04 GAP-002 DONE((b)(c) DEFERRED) · README 상태 · PR→auto-merge→**머지 후 라이브 settings.json 배선**→state bump(53)→드리프트→보고+next-goal(C4=GAP-005)
