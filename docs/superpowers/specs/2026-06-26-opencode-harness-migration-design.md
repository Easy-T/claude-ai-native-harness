...
- **`_oracle/verify-all.sh`** (빌드박스 단일 검증 엔트리, zip 제외): PASS/FAIL 집계로 ① `node --test tests/*.test.mjs`(전 단위) ② `node _oracle/diff-parsers.mjs`(차등 오라클) ③ `node _oracle/skill-discovery.mjs`(≥20 스킬 0위반) ④ **clean-stage 게이트**(`_stage.sh`로 스테이징 → `package.json` 존재+`type:module`·`node_modules`/lockfile **부재**·`plugin/governance.js` node `import()` 로드 OK(오프라인)·skill 수 ≥20·`opencode.json` `skills.urls` 부재). git/node 부재 시 명확한 실패. **STAGE-0 전제**: 첫 줄에서 active plan 전제조건 미충족이면 false-green 금지(CC verify-all STAGE0 선례).
