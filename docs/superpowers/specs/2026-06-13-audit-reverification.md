...
| 6 ✅ | fail-open 표면화 **(cycle-32 완료)** | G3-b·G6-a·failopen-trustbase | M | ✓ | **① rpi-bash.sh:32 종료코드 분기**(`if TARGET=$(node …); then : else hook_log FAILOPEN+stderr; exit 0`) — 크래시(비정상 종료)≠빈출력 구분, fail-open 유지하되 무표면 제거. **② session-start-audit lib/\*.js 런타임 스모크**(`node "$jf" </dev/null` → 손상 파서 ALERT; `bash -n`이 못 잡는 .js 런타임 손상을 차기 세션 표면화 — rank6 실효가치 핵심). **SECURITY.md 신뢰베이스(:19)는 cycle-30 완료**(재작성 X). TDD: RED(현 `\|\|true` 침묵 → PASS=3/FAIL=2) → GREEN(PASS=5/FAIL=0); `setup/tests/failopen-surface.test.sh` 격리 복제본 크래시 스텁, verify-all STAGE 2c, 라이브 cksum witness. **③ defer**(orchestrator ERR-센티넬/secret-scan FAILOPEN 로그): (a) orchestrator ERR-센티넬은 *graceful 파싱실패*만 잡고 *크래시*는 빈 SKEL→no-marker PASS로 우회 → 로그 추가만으론 크래시 클래스 못 잡음(①식 재구조=범위 확대), (b) 그 크래시 클래스는 ②가 이미 선제 표면화, (c) secret-scan은 인라인 self-swallow node라 깨끗한 분기점 없음, (d) G6-b는 §1 non-deficiency·rank6 gapIds 미포함 → 억지 진행 금지 |
