...
- **자기채점 편향 회피**: 핵심 판정은 결정론 신호(exit code·diff·실측) grounding. 루브릭 채점·갭 목록에 교차패밀리(ccs gpt) 적대 리뷰 1회 시도(refute-by-default) — 불가 시 사유 기록 후 진행. *(C10 갱신: "시도→불가 기록"의 canonical 절차는 `docs/ai-context/cross-family-review.md`[2경로 탐지·트리아지·사이클당 1회] — §10 참조.)*
...
**설계 결정**:
1. **규약 SSOT = `docs/ai-context/cross-family-review.md`** (신규 runbook): 탐지(A 우선→B 폴백→SKIP+사유; 설치/로그인 시도 절대 금지)·실행 프로토콜(stdin 파이프·read-only·refute-by-default+원문 인용 강제·메인 세션 트리아지 필수·사이클당 1회)·호출 지점(고-스테이크만: senior review·재채점·적대 리뷰; GPT=추가 발견자≠판정자)·검증자 모델 정책(서브에이전트 model 미지정=세션 상속 기본, 하향 오버라이드는 사유 선언).
2. **codex-plugin-cc 기각(검증 용도)**: ①컨텍스트 공유(`/codex:transfer`·`--resume`)=fresh-context 독립성 오염(반-목표) ②자율 발동(codex-rescue)=사이클당 1회·명시 호출 상한 우회+쓰기 전제(Rule-of-Two 충돌) ③표면 증가(CLI 1개→CLI+플러그인+마켓플레이스; C7 공급망 핀 대상 확장·일단위 고회전) ④스레드 이어가기는 raw CLI 동등(`codex exec resume --last`). 재검토 트리거: 구현-위임 용도 GAP 신설 시 그 용도로만 재평가.
3. **소비처 배선**: closeout-pr-cycle Phase 4 분기(probe→실행+트리아지/SKIP+사유) + start-rpi-cycle 적대 리뷰 앵커 1줄(양쪽 orchestrator 골격 유지·opencode 미러 동기) + 기존 "가능 시" 문구(spec §5·05 §5-6·README 방법론) runbook 앵커 교체.
4. **재채점 무bump 정직**(C7/C8/C9 conjunctive 선례): D2 L5=3 conjunct(교차검증자·CI·자동봉인) 중 1착륙→4 유지, D10 L5="정기 반증"≠규약 신설→4 유지. 04 GAP-006 DONE(증빙=modelUsage gpt-5.6-sol 실측+probe).
5. **스코프 제외 선언**: runbook 토큰 seal(#45+)=GAP-019 편승. 본 리뷰 실전 실행=차기 사이클부터(이 사이클은 규약 등재+probe 스모크만 — quota 규율).
6. **MERGE_POLICY: wait(이 사이클 한정)** — 이니셔티브-와이드 auto보다 goal 문서 선언 우선(사용자 재확인 2026-07-18).
