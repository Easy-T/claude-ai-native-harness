...
## Revision — cycle-23 (2026-06-12): 게이트 수명주기 + fail-open 표면화 + 사이드도어 잔여 봉인

> cycle-22 병행 세션의 전수 감사(explore-strict ×3 + git 포렌식)가 드러낸 **동일 안티패턴의 4개 신규 인스턴스**를 이 durable spec에 in-place 개정으로 수용한다. 근거: 모두 "필수 규칙이 받쳐주는 표면 없이 prose/암묵으로만 존재"하는 본 spec의 Problem 정의에 속한다.

- **D-LIFECYCLE (active plan 의미론 — 명시 Status 필수):** `has_active_plan`의 checkbox-fallback(Status 줄 없음 + 미체크 박스 → active)을 **제거**한다. active = head-20에 `**Status:** active|in_progress` 명시된 plan만.
  - 포렌식 사실: cycle-20 plan은 Closeout step-2(Status flip) 누락으로, cycle-21 plan은 Status 헤더 자체가 없어(fallback 경로) 각각 stale-active — 두 사이클 모두 state.json은 정상 마감(커밋 eb90f58·a25b407에 포함)됐으므로 **중단 잔재가 아니라 Closeout step-2의 silent-skip**. F12(phase-skills)와 동일 클래스의 plan-lifecycle 판.
  - 대안 기각: mtime 14일 캡은 비결정론(아무 편집/touch가 게이트 재연장) + cycle-21형 anomaly를 2주간 방치 → 기각.
  - 3중 표면(채택): ① **seal #27** — 모든 `plans/*.md`에 Status 헤더 존재 **+ active ≤ 1** (verify-setup; 안정 앵커 = head-20 `**Status:**` 토큰 자체) ② **session-start-audit**가 active plan 목록(0/1/N)을 상시 1줄 표시 — stale-active가 다음 세션 시작 즉시 눈에 띔 ③ enforce-rpi-cycle 차단 메시지에 "plan에 `**Status:** active` 헤더가 있는지 확인" 안내 추가(fallback 제거로 생기는 신규 차단의 자기-설명).
  - 데이터 정정: cycle-20 plan Status → completed, cycle-21 plan에 Status 헤더 신설(completed) — 둘 다 state.json이 증명하는 사실의 문서화.
- **D-FAILOPEN-SURFACE (hook 자가점검):** 차단 hook의 fail-open(node 부재→allow, lib 예외→allow, 런타임 에러→non-2 exit→allow)은 세션 벽돌화 방지를 위해 **유지**하되, 고장의 무표면을 제거: `session-start-audit.sh`에 ① `bash -n hooks/*.sh` ② 실행권한 `-x` ③ node 가용성 자가점검을 추가(알림형, 차단 아님 — 기존 30일 audit 알림과 동거). verify-setup에도 `bash -n` 동형 체크(세션 밖 게이트).
- **D-SIDEDOOR-2 (redirect-targets 잔여):** ① 추출가능 타깃 어댑터 추가 — `dd of=`, `install <src> <dst>`, `rsync <src> <dst>`, `cp|mv` **matchAll**(현재 첫 매치만 — `cp a b.md; mv c d.py` 회피 봉쇄) ② `git apply`/`patch`는 타깃이 패치 *내용*에 있어 명령행 추출 불가 → **보수차단**(active plan 부재 시 명령 자체 차단; docs-패치 false-positive 의식 수용, `RPI_SKIP` 탈출구). cases.tsv 동반 케이스 필수.
- **D-TDD-VERBATIM (ultracode (d) TDD 보존):** 옵션 (a)는 plan의 TDD 5-step을 메인이 읽고 위임을 통제하지만, (d)는 stage1 프롬프트가 plan task의 **요약**이면 TDD 단계가 증발한다(execute-strict는 "테스트 없는 코드 변경 → unknowns 보고"의 소극 장치뿐, TDD 강제는 호출자 책임 — agents/execute-strict.md Scope Lock). 수정: start-rpi-cycle (d) 절에 2줄 규칙 — ① stage1 프롬프트에 plan task 본문(TDD 5-step 체크박스 포함)을 **verbatim 전달**(요약 금지) ② stage2 success_criteria에 "RED→GREEN 증거(실패 출력→통과 출력) 없으면 FAIL" 명시. (writing-plans가 TDD를 plan에 인코딩 → verbatim이면 (d)에서도 보존 — 레이어 분석 결과 plan이 유일한 TDD carrier.)
- **D-INSTALL (신선-설치 경로 3결함):** install.sh ① REQUIRED 목록에 `surface-constitution.sh` 추가(doctor·settings.example과 정합 — cycle-16의 본 spec 구현이 누락한 사이트) ② 머지 시 `cur.hooks = tpl.hooks` 통째 덮어쓰기 → 사용자 커스텀 hook 보존 병합으로 ③ stale "hook 8개(9개 등록)" 문구 정정.
- **부수 정정(동일 감사 출처, 1줄급):** agents/review-strict.md "9개 파일"×3 → 13(cycle-19 미도달 사이트); setup/tests/doctor.test.sh Test-4가 git-managed 홈에서 doctor가 안 만드는 백업을 기대 → 수리 후 verify-all STAGE 편입; README 구조트리에 statusline.sh·state.schema.json·SECURITY.md·CONTEXT.md 등재; settings.example.json에 statusLine 블록; superpowers 필수 skill 존재 doctor 알림 체크; cycle-17-goal.md 은퇴 헤더; hooks/.log 월파일 프루닝(doctor).
- **수락 잔여(코드 변경 없음, SECURITY.md 기록):** verify-setup #23 basename-only 비교(matcher drift 미감지) · state.schema.json 무검증(참조 문서로만 유지) · verify-all에서 doctor 선행이 .installed/audit-marker 2쌍을 측정 전 자가치유(의도된 치료-후-검증 순서로 수락) · python -c 변수 파일명 등 Non-Goal 유지.

## Revision — cycle-24 (2026-06-12): 수락 잔여 재평가

> cycle-23이 기록한 수락 잔여 전건을 "결정론적 안정 앵커 + 무오탐 검증식이 생겼는가" 기준으로 재평가. 판정 6건 = ADOPT 1 / KEEP 5.

- **① verify-setup #23 basename-only → ADOPT(승격 — matcher 포함 트리플 parity).** 근거: cycle-23 S3의 install.sh 보존 병합이 isHarness(`/\.claude\/hooks\/[^/]+\.sh/`) 불변식("하네스 hook은 항상 템플릿 entry에만 존재")을 신설 → 하네스 hook 한정 (phase|matcher|basename) 비교가 무오탐 앵커로 성립. RED 실측: 구판은 matcher 변조(`Write|Edit|NotebookEdit`→`Write|Edit`)를 통과(미감지), 승격판은 검출 + 커스텀 hook 추가에 무오탐(출력 불변). 부수 이득: 구판은 사용자 커스텀 hook 추가 시 오탐 FAIL(S3 보존 병합과 비일관)이었으나 승격판은 자연 통과.
- **② state.schema.json 무검증 → KEEP.** 소비자 1(verify-loop-watch.sh)·필드 2의 8줄 참조 문서에 검증자는 과잉 — 새 앵커 없음.
- **③ verify-all doctor-선행 자가치유 → KEEP.** 치료-후-검증은 doctor 설계 자체(§2.7 diagnose-treat-rediagnose) — 순서 변경은 seal 문제가 아니라 설계 변경.
- **④ redirect-targets python -c 변수 파일명 등 → KEEP.** 변수 해석은 정적 파싱의 구조 상한(섀도 실행 필요) — 결정론 앵커 존재 불가(Non-Goal 유지).
- **⑤ §1 stable-claude-md stderr 표면 → KEEP.** 아래 Non-Goals가 기각 사유 기록(사용자-타이밍 관심사라 stderr 정당) — 변동 없음.
- **⑥ (d) 물리 TDD 강제 불가 → KEEP.** PreToolUse는 Workflow stage 프롬프트를 검사 불가(F12급 프롬프트 계약 상한) — cycle-23 TDD-verbatim 규칙이 advisory 상한.
- **부수 drift 정정:** CONTEXT.md "drift seal" 항목의 실재 범위가 #27·#28(cycle-23 신설) 미반영 → `#17~#25·#27·#28`로 정정.
