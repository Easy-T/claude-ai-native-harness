...
# 8. 6 hook scripts executable
for h in enforce-orchestrator stable-claude-md auto-compact-watch enforce-rpi-cycle enforce-rpi-bash session-start-audit; do
...
# 14. settings.json has >=6 hooks registered (3 PreToolUse Write|Edit|NotebookEdit + 1 Bash + 1 PostToolUse + 1 SessionStart)
COUNT=$(node -e '
...
' 2>/dev/null || echo 0)
[ "$COUNT" -ge 6 ] && ok "settings.json: $COUNT hooks" || fail "settings.json hooks=$COUNT"
