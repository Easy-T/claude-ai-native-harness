...
- 하나 이상의 완료된 RPI 사이클 존재 (state.json cycle.count > 0)

없으면: "도메인 문서가 부족합니다. RPIC를 최소 1회 완료 후 실행하세요." 보고 후 중단.

# Phase 1 — Exploration

Dispatch the **explore-strict** subagent via the `task` tool —
...
대상 코드베이스가 **스캐폴드 registry를 가진 경우**(`docs/ai-context/scaffold-registry.md`), Candidate 목록에 **노화 스캐폴드 프루닝 후보**도 포함한다:
- registry의 각 구성요소(hook/skill/seal)의 "존재 이유"가 여전히 유효한지 점검 — 모델 업그레이드로 무용해진 가드·중복 봉인·미발화 케이스.
- 후보는 `[P<n>] 프루닝: <구성요소> — <존재 이유>가 <사유>로 무용 의심 — 제거 시 영향 <검증 커맨드>` 형식.
- ⚠️ **삭제는 자동 실행 금지** — 후보 *보고*만. 실제 제거는 사용자 승인 후 별도 사이클(seal·테스트 동반).
- 근거: 스캐폴드는 제거 트리거 없으면 노화(GAP-005). registry + 이 단계가 트리거.

# Phase 3 — Execution

선택된 각 candidate에 대해 순서대로 the **execute-strict** subagent를 `task` 도구로 디스패치:
- task: "<candidate [N] 설명>"
- read: <관련 파일 경로>
- success:
  - 리팩토링 완료
...
## 디스패치 형식 참고 (orchestrator skeleton — load-bearing)

이 스킬의 서브에이전트 디스패치는 위 Phase들에서 `task` 도구 prose로 표현되지만,
오케스트레이터 게이트(plugin/gates/orchestrator-gate.js)가 인식하는 정식(canonical) 형태는 다음과 같다:

```
Agent(subagent_type="review-strict",
      task="<candidate 변경에 대한 senior review>",
      context_paths=[<변경 파일 경로>],
      success_criteria="Critical 0")
```

(opencode: dispatch the review-strict subagent via the task tool — task: 변경 검토; read: 변경 파일; success: Critical 0.)
