...
1. **Pretendard** CSS import (§2의 CDN URL)
2. **Tailwind config** — §1의 `primary` / `neutral` 컬러 토큰 + §2의 `fontFamily.sans = ['Pretendard', ...]` (§2 CDN이 선언하는 family명은 `'Pretendard'` — `'Pretendard Variable'`은 미선언이라 silent 폰트 폴백. // evidence: F-XR-02)
3. **Coolicons** — React는 `react-coolicons` npm, 비-React는 webfont 자가호스팅 (§5 — CDN `@import`는 작동하지 않음. // evidence: F-XR-01)
4. **currentColor 보정 CSS** — `svg path[fill="black"] { fill: currentColor; }` (§5 — 없으면 다크/잉크 표면에서 아이콘 소실. // evidence: F-XR-01, F-L1-11)

이 4개가 누락되면 `bg-primary`, `text-neutral-900`, `font-sans`, 아이콘이 무효/소실. 컴포넌트 단위 작업 시 먼저 확인.
...
:root {
  ...
  --color-scrim: hsl(220, 10%, 10%);   /* 모달 오버레이 전용 — 아래 scrim 예외 참조 // evidence: F-XR-03 */
}
```
...
html[data-theme="dark"] {
  ...
  --color-scrim: hsl(220, 14%, 4%);    /* 다크에서 더 깊게 — "흰 스크림" 역전 방지 // evidence: F-XR-03 */
}
```
...
- **scrim 예외**: 모달 오버레이는 `bg-neutral-900/40`을 쓰면 다크에서 "흰 스크림"으로 역전 → 의미 토큰 `--color-scrim`(라이트 `hsl(220,10%,10%)` / 다크 `hsl(220,14%,4%)`)을 **별도 백드롭 레이어**(`bg-scrim opacity-40`)로 사용. ⚠ `bg-scrim/40` 같은 알파 수정자는 이 config(색=`var()` 문자열)에서 조용히 컴파일 탈락한다 — `<alpha-value>` 미주입. // evidence: F-XR-03, F-XR-11
...
# 2. Typography
한글과 영문 모두 자연스럽게 어우러지는 **Pretendard** 단일 폰트를 사용합니다 (아래 CDN이 선언하는 family명 기준 — `'Pretendard Variable'` 아님. // evidence: F-XR-02). // from: Pretendard 가이드
...
**한국어 장문 measure (v2):** 아티클 본문 폭은 **32–38em** (17px 기준 한글 30자대 중반). 라틴 조판 관용(행당 60–70자)을 한글 자수로 그대로 옮기면 과폭 — 한글 음절은 정사각 밀도라 같은 자수에서 행이 훨씬 길다. long-form 본문은 17–18px / line-height 1.7–1.9 티어 사용 (§2 표의 body-*는 UI 문장용). // evidence: F-L2-03, F-L2-04, F-XR-09
...
<!-- py-2.5 실높이 ≈41px — 모바일 "주 액션" 버튼은 py-3(≥44px, §13 hit target)로. // evidence: F-XR-06 -->
...
<div class="flex flex-col gap-1.5">
  ...
  <input type="email" class="px-4 py-2.5 bg-neutral-0 border border-neutral-300 rounded-lg text-body-lg md:text-body-md text-neutral-900 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder:text-neutral-500" placeholder="user@example.com" />
</div>
```
input 텍스트는 모바일 16px(`text-body-lg`) — 14px이면 iOS가 포커스 시 자동 줌(§13 플로어). 데스크톱은 `md:text-body-md`로 복귀 (§8 Form View 인풋도 동일 적용). // evidence: F-XR-05
...
### Modal
닫기 버튼은 오직 1개만 배치합니다. 오버레이는 scrim 토큰의 **별도 백드롭 레이어**(`bg-scrim opacity-40` — §1 scrim 예외; `bg-neutral-900/40`은 다크에서 흰 스크림으로 역전, `bg-scrim/40` 알파 수정자는 var() config에서 컴파일 탈락), 푸터 버튼 페어는 위 Button Pair 규칙(동일 너비)을 따릅니다. // evidence: F-XR-03, F-XR-04, F-XR-11
```html
<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
  <div class="absolute inset-0 bg-scrim opacity-40" aria-hidden="true"></div><!-- 백드롭 레이어 -->
  <div class="relative w-full max-w-sm bg-neutral-0 rounded-2xl p-6 shadow-lg">
    ...
    <div class="flex gap-2">
      <button class="flex-1 px-4 py-2 bg-neutral-100 text-neutral-900 rounded-lg font-medium">취소</button>
      <button class="flex-1 px-4 py-2 bg-primary text-neutral-0 rounded-lg font-medium">확인</button>
    </div>
  </div>
</div>
```
...
```
```tsx
import { Search, CloseBig, Hamburger, User, Sun, Moon, LongRight } from "react-coolicons";
// 색: path fill="black" 하드코딩 — 아래 currentColor 보정 CSS 필수. 크기는 width/height.
<Search width={20} height={20} />
```
정확한 export명은 `node_modules/react-coolicons/esm/` 디렉터리로 확인. v2 정정 — 실물 export명 (before: `SearchMagnifyingGlass`·`CloseLg`·`User01`로 안내했으나 **존재하지 않는 export라 첫 렌더부터 크래시**): `ci-search`→`Search`, `ci-close_big`→`CloseBig`, `ci-hamburger`→`Hamburger`, `ci-user`→`User`, `ci-warning`→`Warning`, 테마→`Sun`/`Moon`, 화살→`LongRight`/`LongDown`, `ci-chevron_*`→`ChevronBig*`. 아래 §Semantic Icon Mapping의 `ci-*`명은 *의미 참조*이며, React에선 대응 컴포넌트로 매핑한다. // evidence: F-L3-05
...
```tsx
<li>
  <button type="button" className="flex w-full items-center px-4 py-3 bg-neutral-0 active:bg-neutral-100 transition-colors text-left">
    <span className="w-10 h-10 rounded-full bg-neutral-100 flex items-center justify-center text-neutral-500 mr-3">
      <i className="ci-user text-[20px]"></i>
    </span>
    <span className="flex-1 min-w-0">
      <span className="block text-body-md font-medium text-neutral-900 truncate">홍길동</span>
      <span className="block text-body-sm text-neutral-500 truncate">hong@example.com</span>
    </span>
    <i className="ci-chevron_right text-neutral-300 text-[20px] ml-2"></i>
  </button>
</li>
```
행 전체가 클릭 대상이므로 마크업도 button(이동이면 a) — plain `<li>`는 키보드 포커스·조작 시맨틱이 없어 "인터랙티브 행 전제"와 불일치. focus-visible은 §12 레시피. // evidence: F-XR-10
...
<form className="w-full max-w-sm mx-auto p-6 bg-neutral-0 rounded-xl border border-neutral-300 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
  ...
  <div className="flex flex-col gap-4 mb-6">
    <div className="flex flex-col gap-1.5">
      ...
      <input type="email" className="px-4 py-2.5 bg-neutral-0 border border-neutral-300 rounded-lg text-body-lg md:text-body-md focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-neutral-500" placeholder="이메일을 입력하세요" />
    </div>
    <div className="flex flex-col gap-1.5">
      <label className="text-body-sm font-medium text-neutral-700">비밀번호</label>
      <input type="password" className="px-4 py-2.5 bg-neutral-0 border border-neutral-300 rounded-lg text-body-lg md:text-body-md focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-neutral-500" placeholder="비밀번호를 입력하세요" />
    </div>
  </div>
...
- **stagger**: 순차 등장은 60–120ms 간격 (KPI 카드 60ms·hero 행 120ms 실측). **안무 예산은 2축**이다 — *개별 요소 지속* ≤700ms(motion-hero 상한)와 *마지막 요소의 시작 지연* ≤300ms는 별개 축 — **지연 축이 지배**한다. **마지막 지연 = 간격 × (N−1)**: N요소면 간격을 내려 이 값이 ≤300ms가 되게 한다(예: 5요소×120ms 간격 → 120×4=480ms 위반, ≤75ms 간격 → 75×4=300ms OK). 요소가 많으면 60ms 하한보다 ≤300ms 축이 우선(60ms 미만 허용 — 7요소면 ≤50ms). // evidence: F-AUD-02, F-XR-07, F-XR-08 체감 총 길이 = 개별 지속 + 시작 지연 spread ≤ **~1000ms**. (v2 "총 안무 <700ms"는 700ms line-rise 레시피와 모순 — 개별 지속과 시퀀스 spread를 혼동했음.) // evidence: F-FIT-02
