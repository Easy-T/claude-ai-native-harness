...
description: |
  ...
  trivial 변경(≤5라인 수정)은 예외 — 거버넌스 플러그인의 RPI 게이트(plugin/gates/*.js)가 자동 통과시킴.
...
A. `brainstorming` 스킬을 `skill` 도구로 호출 (메인이 직접 따름) — 외향적 (요구·접근법·디자인)
...
B. `grill-with-docs` 스킬을 `skill` 도구로 호출 (메인이 직접 따름) — A의 design을 도메인 모델·코드에 비춰 stress-test
   ※ grill-with-docs는 번들에 vendored된 네이티브 skill — 항상 사용 가능 (별도 설치 불필요)
...
C. **explore-strict** 서브에이전트를 `task` 도구로 디스패치 —
   task: "<요청 분석>";
   read: ["CONTEXT.md",
   ...
   success: "발견사항·영향 모듈·신규 도메인 용어·deny pattern 충돌 식별".
   ※ AGENTS.md는 메인이 자동 로드하므로 read 목록에 미포함 (중복 회피)
...
`writing-plans` 스킬을 `skill` 도구로 호출 (메인이 직접) → docs/superpowers/plans/YYYY-MM-DD-<topic>.md
...
1. active plan 파일 존재 확인 (거버넌스 플러그인의 RPI 게이트(plugin/gates/*.js)가 의존)

2. **review-strict** 서브에이전트를 `task` 도구로 디스패치 —
...
- (d) **순차 execute-strict→review-strict (task별)** (하네스가 이미 사용 — opencode에는 Workflow 도구가 없음) —
      Phase I 한정(R/Closeout 병렬화는 ceremony라 제외). plan task를 canonical 2-stage 파이프라인으로:
      stage1 **execute-strict** 서브에이전트(구현) → stage2 **review-strict** 서브에이전트(검증).
      ※ 두 스테이지 모두 **schema 금지** — 제약된 wrapper 서브에이전트는 StructuredOutput 부재로 schema와 함께 실패 ([[feedback_workflow_agenttype_schema]] 교훈; schema 복원 유혹 금지).
...
다음 조건을 모두 충족하면 `closeout-pr-cycle` 스킬을 `skill` 도구로 호출:
...
3. state.json (harness-relative, 번들 루트에 ship) 갱신 (메인이 jq 또는 node로 read-modify-write):
...
7. 다음 사이클 goal 초안 (advisory — cycle.count ≥ 1일 때 필수; 출력 = Communication Protocol `next-cycle-goal` **고유 필수 필드**):
   ...
   ② `read-before:` post-compact 필수 읽기 문서 — `/compact` 가정. 다음 사이클 진입 전 반드시 읽을 문서를 *존재하는 것만* 절대경로로 나열:
      ...
      직전 plan · docs/ai-context/non-obvious.md · 관련 AGENTS.md §섹션.
...
- paused: 일시 중지 (거버넌스 플러그인의 RPI 게이트가 active로 인식 안 함)
...
- harness-verify: **고유 필수 필드** (모든 사이클). verify-setup PASS를 복합 evidence에 접지 않고 별도 표면화 — 정확히 아래 중 하나:
  · 이번 사이클이 ~/.config/opencode(opencode 하네스)를 수정 → `PASS — tests + oracles green` (Step C-1 sub-step 6 실행 결과).
  · 비-하네스 사이클 → "N/A — 이번 사이클은 opencode 하네스를 수정하지 않음".
